...
export function serializeRoomOp(socket, op) {
    const next = (socket.data.roomOps ?? Promise.resolve())
        .then(op)
        .catch((err) => console.error("[room] handler error:", err?.message ?? err));
...
const departureKey = (roomId, userId) => `${roomId}:${userId}`;
function cancelPendingDeparture(roomId, userId) {
...
function scheduleDeparture(io, roomId, userId, username) {
    ...
    const timer = setTimeout(async () => {
        ...
        try {
            ...
            if (await RoomManager.hasUser(roomId, userId))
                return;
            io.to(roomId).emit("room:user-left", { userId, username });
            console.log(`[room] ${username} left ${roomId} — ${await RoomManager.getUserCount(roomId)} remaining`);
        }
        catch (err) {
 ...
 */
async function releaseDocIfIdle(roomId) {
    if (RoomManager.localSocketCount(roomId) > 0)
        return;
 ...
 */
export async function handleLeave(io, socket, userId, username, roomId, leaveSocketRoom) {
...
export function registerRoomHandlers(io, socket) {
    ...
    // ── room:join ─────────────────────────────────────────
    socket.on("room:join", ({ roomId }) => serializeRoomOp(socket, async () => {
    ...
    // ── room:leave ─────────────────────────────────────────
    socket.on("room:leave", ({ roomId }) => serializeRoomOp(socket, async () => {
        await socket.data.ready;
        const username = socket.data.username;
        await handleLeave(io, socket, userId, username, roomId, true);
    }));
    // ── room:language-change ───────────────────────────────
    socket.on("room:language-change", async ({ roomId, language }) => {
        await socket.data.ready;
        const username = socket.data.username;
...
async function sendRoomState(socket, roomId, userId, language) {
