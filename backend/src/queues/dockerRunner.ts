...
// ── Language → Docker config ──────────────────────────────
// `cmd` is a shell snippet run after the code has been written to `file`
// (always under /tmp, the container's only writable path — see runInDocker).
...
let lastDockerOk: boolean | null = null;
export function isDockerAvailable(): boolean | null { return lastDockerOk; }
...
export async function runInDocker(
  code:       string,
  language:   string,
  timeoutMs = EXEC_LIMITS.timeoutMs,
): Promise<ExecResult> {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (result: ExecResult, killContainer: boolean) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      resolve(result);
      if (killContainer) void removeContainer(name);
    };
    ...
    const onData = (stream: "stdout" | "stderr") => (d: Buffer) => {
      if (settled) return;
      outBytes += d.length;
      if (stream === "stdout") stdout += d.toString(); else stderr += d.toString();
    ...
    proc.on("close", (code) => {
      ...
      const note     = (msg: string) => { err += (err && !err.endsWith("\n") ? "\n" : "") + msg; };
      ...
      if (exitCode === 137) note(`Killed — memory limit exceeded (${EXEC_LIMITS.memoryMb} MB).`);
      if (exitCode === 152) note(`Killed — CPU time limit exceeded (${cpuSecs}s).`);
      finish({
        stdout:          stdout.slice(0, STDOUT_SHOW),
        stderr:          err,
        exitCode,
...
export async function reapOrphanContainers(): Promise<number> {
  ...
  if (stale.length) await docker(["rm", "-f", ...stale]);
  return stale.length;
}

/** Sweeps once immediately, then every `intervalMs`. Never keeps the process alive. */
export function startContainerReaper(intervalMs = 60_000): NodeJS.Timeout {
  const sweep = () =>
    reapOrphanContainers()
      .then((n) => { if (n) console.log(`[worker] Reaped ${n} orphaned exec container(s)`); })
